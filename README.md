...
UVM-based functional verification of a 6-stage RISC-V SIMT core pipeline, with focus on warp scheduling, data hazards, operand collection, execution units, and SIMT-visible system mechanisms such as barriers and memory ordering fences.
...
- **Stimulus** generation combines directed and constrained-random sequences targeting specific SIMT features. A custom RISC-V instruction generator produces architecturally valid instruction streams, which are packaged into cacheline-aligned transactions and delivered to the core through a memory BFM and instruction cache interface.
...
- A **memory BFM** provides a deterministic, controllable interface for instruction and data access. It serves as a verification abstraction rather than a full system-level memory model, enabling precise stimulus control and observability without requiring a complete memory subsystem.

- **Functional coverage** is intent-driven and used to measure exploration of warp states, scheduling decisions, hazards, synchronization behavior, and execution outcomes. Coverage results guide stimulus refinement rather than serving as a purely quantitative metric.

Vortex implements SIMT behavior using a set of **custom RISC-V ISA extensions** (e.g., warp control, divergence, synchronization instructions). These extensions do not have a standalone architectural specification; instead, the decoder and surrounding RTL behavior serve as the primary reference for intended functionality. Verification correctness is therefore established through consistent engineering judgment applied across execution behavior, scheduling, and observable SIMT semantics.

The environment is modular by construction, allowing verification scope to scale from IP-level execution blocks to subsystem-level integration through configuration and composition.
...
Refer to [`docs/rtl_docs/microarchitecture.md`](docs/rtl_docs/microarchitecture.md) and  
[`docs/rtl_docs/assets/vortex_microarchitecture.png`](docs/rtl_docs/assets/vortex_microarchitecture.png) for full micro-architectural details.
...
- `/docs`       : Validation plan, bug reports, coverage notes
